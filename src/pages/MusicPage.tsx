...
const MusicPage = () => {
  ...
  if (reducedMotion) {
    return (
      <PageShell title="Music" subtitle="Ambient player disabled when reduced motion is preferred." accent="emerald">
        <p className="p-6 text-sm text-readable-muted">Enable motion in your system settings to use the music player.</p>
      </PageShell>
    );
  }
  ...
  return (
    <PageShell
      title="Music Studio"
      subtitle="Search and stream focus tracks from YouTube, preview clips from Deezer, or paste any YouTube link. Built for deep-work sessions while you explore the portfolio."
      accent="emerald"
    >
      <div className="grid lg:grid-cols-[1fr_1.1fr] gap-0 min-h-[min(72vh,640px)]">
        <div className="flex flex-col border-b lg:border-b-0 lg:border-r border-white/5 font-mono">
          <div className="p-4 sm:p-5 border-b border-white/5 space-y-3">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search songs, artists, cyber ambient, lofi…"
            ...
            {results.length > 0 ? (
              ...
              <div>
                <p className="text-[10px] text-slate/40 uppercase tracking-wider px-1 mb-3 flex items-center gap-2">
                  <Radio className="w-3 h-3" /> Curated stations
                </p>
                <ul className="space-y-1">
          ...
          <div className="p-4 border-t border-white/5 flex items-center gap-3 shrink-0">
            <button
              type="button"
              onClick={togglePlay}
              className="p-2.5 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-400"
        ...
        <div className="flex flex-col bg-black/30 min-h-[280px]">
          <div className="p-4 border-b border-white/5">
            <p className="text-[10px] uppercase tracking-widest text-emerald-400/70">Now playing</p>
            <p className="text-lg text-white font-medium mt-1 truncate">{current?.title ?? 'Select a track'}</p>
            <p className="text-xs text-slate/50 truncate">{current?.artist ?? 'Search or pick a curated station'}</p>
          </div>
          <div className="flex-1 flex items-center justify-center p-4">
            {ytSrc && mode === 'youtube' ? (
              <div className="w-full aspect-video max-w-xl rounded-xl overflow-hidden border border-white/10 shadow-2xl">
                <iframe
                  title="YouTube music"
                  src={ytSrc}
              ...
              <div className="text-center space-y-4">
                {current.thumbnail && (
                  <img src={current.thumbnail} alt="" className="w-40 h-40 rounded-2xl object-cover mx-auto shadow-xl" />
                )}
                <p className="text-sm text-slate/60">Deezer 30s preview loop</p>
              </div>
            ) : (
              <div className="text-center text-slate/40 text-sm max-w-xs">
                <Music className="w-12 h-12 mx-auto mb-3 opacity-30" />
                Pick a track from search or curated stations to start playback.
              </div>
            )}
          </div>
        </div>
